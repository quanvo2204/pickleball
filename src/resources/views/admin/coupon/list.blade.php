            ...
            <div class="card">
                ...
                <div class="card-body table-responsive p-0">
                    <table class="table table-hover text-nowrap">
                        ...
                        <tbody>
                            @foreach ($discountCoupon as $discountCoupons)
                                <tr>
                                    <td>{{ $discountCoupons -> id  }}</td>
                                    <td>{{ $discountCoupons -> code }}</td>
                                    <td>{{ $discountCoupons -> name }}</td>
                                    @if ($discountCoupons->type == 'percent')
                                        <td>{{ $discountCoupons -> discount_amount }}%</td>
                                    @else
                                        <td>${{ $discountCoupons -> discount_amount }}</td>
                                    @endif

                                    <td>{{ $discountCoupons -> max_uses }}</td>


                                    <td>{{ (!empty($discountCoupons -> start_at)) ? \Carbon\Carbon::parse($discountCoupons -> start_at)->format('Y/m/d H:is') : ''}}</td>
                                    <td>{{ (!empty($discountCoupons -> expires_at)) ? \Carbon\Carbon::parse($discountCoupons -> expires_at)->format('Y/m/d H:is') : ''}}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('coupons.edit', $discountCoupons->id) }}">
                                        ...
                                        <a href="" data-id="{{ $discountCoupons->id }}"
                                            class="text-danger w-4 h-4 mr-1 delete-coupon">
                ...
                <div class="card-footer clearfix">
                    ...
                    <div class="float-right">
                        {{ $discountCoupon->links('pagination::bootstrap-4') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            $('.delete-coupon').on('click', function(e) {
                ...
                let couponId = $(this).data('id'); // lấy giá trị của thuộc tính data-id
